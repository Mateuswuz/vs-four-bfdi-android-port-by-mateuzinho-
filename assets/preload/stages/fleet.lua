...
function onCreate()
	...
	addCharacterToList('honey', 'boyfriend')
	precacheSound('stat')

	setPropertyFromClass('GameOverSubstate', 'characterName', 'coiny bfb')
end
...
function fleetON()
	...
	setProperty('coiny bfb.alpha', 0)
	doTweenAlpha('fleetON', 'fx', 0, 1, 'linear')
	playSound('stat', 0.3)
	triggerEvent('Change Character', 0, 'honey')
end

function fleetOFF()
	setProperty('fx.alpha', 0.8)
	setProperty('coiny bfb.alpha', 1)
	doTweenAlpha('fleetON', 'fx', 0, 1, 'linear')
	playSound('stat', 0.3)
	triggerEvent('Change Character', 0, 'coiny bfb')
end
